...
interface UseValidMoveReturn {
  validMove: (args: MakeMoveArgs) => Promise<void>;
  isPending: boolean;
  ...
  const validMove = async ({ fromX, fromY, toX, toY, units }: MakeMoveArgs) => {
    try {
      ...
      await writeContract({
        ...
        functionName: "checkValidMove", // Function name to call
